...
{
    [TestClass]
    public class CuentasControllesTests : BasePruebas
    {
        [TestMethod]
        public async Task CrearUsuario()
        {
            ...
            var conteo = await _context2.Users.CountAsync();
        ...
        public async Task UsuarioNoPuedeLogearse()
        {
            ...
            var userInfo = new CredencialesUsuarios() { Email = "ejemplo@ejemplo.com", Password = "Aasmin123!" };

            var respuesta = await controller.Login(userInfo);
            Assert.IsNull(respuesta.Value);

            var resultado = respuesta.Result as BadRequestResult;
        ...
        public async Task UsuarioPuedeLogearse()
        {
            ...
            var controller = ConstruirCuentasController(nombreDB);
            var userInfo = new CredencialesUsuarios() { Email = "ejemplo@ejemplo.com", Password = "Admin123!" };
        ...
        private CuentasController ConstruirCuentasController(string nombreBD)
        {
            ...
            return new CuentasController(userManager, configuration, signInManager, null, context, mapper);
        }
